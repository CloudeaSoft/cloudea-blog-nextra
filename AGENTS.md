...
- **A `.env` file is required for `pnpm build` (and correct dev metadata).** Site URL helpers live in `utils/env.ts` (`getBaseUrl`, `getBasePath`, `getSiteUrl`). With `NEXT_PUBLIC_BASE_URL` unset, `getSiteUrl()` is invalid for `new URL(...)`, so the build fails with `TypeError: Invalid URL` while collecting the `/categories` pages. `.env` is gitignored; create it once from `.env.example` (`NEXT_PUBLIC_BASE_URL="http://localhost:3000"`, `NEXT_PUBLIC_BASE_PATH=""`) if it is missing.
