...
- **Lint:** there is no `lint` script; run ESLint directly with `npx eslint .`. As of setup it reports 2 pre-existing errors in `mdx-components.tsx` and 1 warning in `app/_components/layout/sidebar.tsx` (not caused by env setup).
